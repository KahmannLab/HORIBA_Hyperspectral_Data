...
class JobinYvonXMLReader:
    ...
    def get_data(self):
        ...
        if num_rows == 0:
            ...
            for i, row in enumerate(data_raw):
                ...
                # Check if row_array length matches num_cols
                if len(row_array) != num_cols:
                    _logger.warning(
                        f"Row {i} has length {len(row_array)}, expected {num_cols}. Adjusting the row size.")
                    if len(row_array) > num_cols:
                        # Trim the array if it's too long
                        row_array = row_array[:num_cols]
                    else:
                        # Pad the array with zeros if it's too short
                        row_array = np.pad(row_array, (0, num_cols - len(row_array)), 'constant')
    ...
    def map_metadata(self):
        ...
        laser["Filter"]["optical density"] = self.original_metadata[ #change optical density to filter
            "experimental_setup"
        ].get("Filter") #change ND Filter (%) to Filter
        ...
        #try:
        #    laser["Filter"]["optical density"] /= 100
        #except KeyError:  # pragma: no cover
        #    pass  # pragma: no cover
        ...
        try:
            ...
            spectrometer["slit"] /= 100
            spectrometer["pinhole"] = spectrometer["slit"]
        except KeyError:  # pragma: no cover
            pass  # pragma: no cover
